...
class SimulationParameterExtracter():
    ...
    def plotFittedExp(self):
        ...
        mTau = np.mean(self.fitDetails['timeConstants'])

        vTraces = []
        for seg in self.CCData.segments:
            vTraces.extend(seg.analogsignals)

        for (pOpts, yData, bestWindow) in zip(self.fitDetails['pOpts'], self.CCData.segments,
                                              self.fitDetails['bestWindow']):
            pOpt = pOpts[bestWindow]
            plt.cla()
            tVecShifted = np.arange(len(yData) - 10) * yData.sampling_period.magnitude
            yFitted = funcToFit(tVecShifted, pOpt[0], pOpt[1], pOpt[2])
            plt.plot(yData[10:].magnitude, 'r')
            plt.plot(yFitted, 'g')
            plt.title(str([bestWindow, mTau, 1 / pOpt[2]]))
            plt.draw()
            ch = 'n'
            ch = raw_input('Next(n)/Quit(q):')
            if ch == 'q':

...
standFunc = testMorph.getStdFunctionPCA1()
...
np.savetxt(swcfName[:swcfName.index('.')]+'_UFSTD.swc', swcData,'%d %d %0.6f %0.6f %0.6f %0.6f %d',
            header=headr, comments='#')

marksFilePath = swcfName[:-4] + '.marks'
if os.path.isfile(marksFilePath):
    shutil.copy(marksFilePath, swcfName[:-4] + '_UFSTD.marks')